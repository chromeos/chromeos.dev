...
const COMBINED_REGEX = new RegExp(
  `%\\[(${SINGLE_REGEX.source}{1}|${MULTIPLE_REGEX.source})\\]`,
);
...
function statsRenderer(md) {
  return function statsRender(tokens, idx) {
    ...
    let stats =
      '<figure class="stats stats--renderer"><dl class="stats__list">';
    const defaultShapes = ['semicircle', 'circle', 'triangle'];
    for (const [i, stat] of all.stats.entries()) {
      stats += `<div class="stats__item"><dt class="stats__stat ${
        i < defaultShapes.length ? `stats__stat--${defaultShapes[i]}` : null
      }"><span>${
        stat[0]
      }</span></dt><dd class="stats__desc stats__desc--renderer type--base">${
        stat[1]
      }</dd></div>`;
    }
    stats += `</dl></figure>`;
...
function cleanSingleStat(stat) {
  ...
  const desc = stat.replace(statAlone, '');
  output.push(desc.replace(replaceRegex, ''));
...
function statsRuler(md) {
  return function statsTokenize(state, silent) {
    ...
    const match = COMBINED_REGEX.exec(
 ...
 * @param {object} md - Markdown-It object
 */
export function statsPlugin(md) {
