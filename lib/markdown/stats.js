...
function statsRenderer(md) {
  return function statsRender(tokens, idx) {
    ...
    let stats = '<figure class="stats"><dl class="stats__list">';
    for (const stat of all.stats) {
      stats += `<div class="stats__item"><dt class="stats__stat"><span>${stat[0]}</span></dt><dd class="stats__desc type--h5">${stat[1]}</dd></div>`;
    }
    stats += `</dl></figure>`;
