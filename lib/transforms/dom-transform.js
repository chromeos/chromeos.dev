...
// Storage things
const storageBase = 'https://storage.googleapis.com/cros-staging.appspot.com/';
...
function buildGoogleStorageImage($, el) {
  ...
  const base = src.replace('gs://', '');
  const url = storageBase + base;
  const newSrc = `${url}.${image.cuts.sort((a, b) => a - b)[Math.round((image.cuts.length - 1) / 2)]}${image.types.filter(t => t.extension !== '.webp')[0].extension}`;

  const sources = image.types
    .map(type => {
      let source = `<source sizes="${image.sizes}" type="${type.type}"`;
      source += ` srcset="${image.cuts.map(c => `${url}.${c}${type.extension} ${c}w`).join(', ')}"`;
      source += '>';
      return source;
    })
    .join('\n');
