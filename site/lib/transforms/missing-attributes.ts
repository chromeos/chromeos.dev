import slug from 'uslug';
import select from 'posthtml-match-helper';
import { addClass, setAttribute, getChildrenByTag } from './_posthtml';
 ...
 * @return {PostHTMLTree}
 */
export async function postHTMLMissingAttributes(tree) {
  ...
  // Add IDs to terms
  tree.match(select('dt'), (node) =>
    setAttribute('id', slug(node.content?.[0]))(node),
  );
  ...
  tree.match(select('.code-figure'), (node) => {
    addClass('code-figure--caption type--label')(
      getChildrenByTag('figcaption', node),
    );
  ...
  tree.walk((node) => {
    if (node?.content?.length) {
      const link = node.content.find(
        (child) =>
          child?.tag === 'a' && child?.attrs?.class?.includes('header-anchor'),
      );
      if (link) {
